...
bov_points_t* bov_points_new_with_value(const GLfloat data[][3],
                             GLsizei n,
                             GLenum usage)
{
	...
	points->param = (bov_points_param_t) {
		...
		.outlineWidth = -1.0f,
		.spaceType = USUAL_SPACE,
		.colormap = 1							// 0 for bov_points_new
	};
	...
	// specify the layout of the data
	glVertexAttribPointer(POS_LOCATION, 2, GL_FLOAT, GL_FALSE, 3*sizeof(GLfloat), 0);
	glEnableVertexAttribArray(POS_LOCATION);

	glVertexAttribPointer(DATA_LOCATION, 1, GL_FLOAT, GL_FALSE, 3*sizeof(GLfloat), (void*)(2*sizeof(GLfloat)));
	glEnableVertexAttribArray(DATA_LOCATION);

	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * 3 * n, data, usage);
